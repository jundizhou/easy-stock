...
| `GET` | `/api/v1/stocks/chan-screen-status` | Availability of the chan.py (Vespa314) 缠论引擎 service: `available`, script path and `reason` when the local `chanpy-service` deployment is missing. |
| `GET` `POST` | `/api/v1/stocks/chan-screen` | 缠论选股：batch-screen a stock pool with the chan.py engine. Body: `symbols`(≤200), `names`, `period`, `limit`, `filters{side,bs_types,zs_state,bi_direction,bsp_recent_bars,min_score,require_sure}`. Returns per-stock buy/sell points, pivot position, stroke direction and a deterministic score; results sorted matched-first. Long request (budget 150s). |
| `GET` `POST` | `/api/v1/stocks/chanpy-analysis` | chan.py 缠论结构分析 for one stock: full bi / segment / pivot / buy-sell-point lists plus the same score summary. Body or query: `symbol`, `period`, `limit`, optional `conf` overrides. |
