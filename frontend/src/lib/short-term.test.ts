...
describe('short-term theme helpers', () => {
	...
	it('sorts themes by selected strength before source-balanced ranking', () => {
		const first: ThemeOverview = {
			...
			top_node_change_percent: 0, trend_score: 70, daily_strength_score: 65, source_rank: 1,
		};
		const industry: ThemeOverview = {
			theme: 'compute_rental', name: '算力租赁', change_percent: 0, main_net_inflow: 0,
			rising_nodes: 0, falling_nodes: 0, matched_nodes: 0, total_nodes: 0,
			top_node_change_percent: 0, trend_score: 99, daily_strength_score: 88, daily_rank: 2,
		};
		const third: ThemeOverview = {
			theme: 'kpl:2', name: '芯片', change_percent: 0, main_net_inflow: 0,
			rising_nodes: 0, falling_nodes: 0, matched_nodes: 0, total_nodes: 0,
			top_node_change_percent: 0, trend_score: 90, daily_strength_score: 82, source_rank: 3,
		};
		expect(rankThemeOverviews([first, industry, third]).map((item) => item.name)).toEqual(['算力租赁', '芯片', '通信']);
	});
